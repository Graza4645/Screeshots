 ...
 * (With Scroll — scrolls breakup section before capture so hidden rows like
 *  Employer's Gratuity Contribution, GMC & GPA, GTLI, Total Benefits are visible)
 *
...
const TF_DIR = path.resolve('tests/Hiring_Tabulation_SalaryCalculator/screenshots/TotalBenefits');
...
async function takeTFScreenshot(page, name) {
    ...
    // Scroll to Total Benefits so it is visible in the screenshot
    await page.evaluate(() => {
        ...
        for (const row of hlRows) {
            ...
            if (label && label.textContent.trim() === 'Total Benefits') {
    ...
    await page.waitForTimeout(300);
    console.log(`📸 Total Benefits Screenshot: screenshots/TotalBenefits/${filename}`);
}
...
/**
 * Scroll down until the "Total Benefits" row is visible in the viewport.
 * This ensures all hidden rows above it (Gratuity, GMC & GPA, GTLI) are also rendered.
 */
async function scrollToTotalBenefits(page) {
    // First scroll to top to render top rows
    await page.evaluate(() => { window.scrollTo(0, 0); });
    await page.waitForTimeout(300);

    // Scroll down in small steps until "Total Benefits" highlight row is in the viewport
    ...
    for (let i = 0; i < maxAttempts; i++) {
        const found = await page.evaluate(() => {
            ...
            for (const row of hlRows) {
                ...
                if (label && label.textContent.trim() === 'Total Benefits') {
                    ...
                    // If found but not in viewport, scroll it into view
                    row.scrollIntoView({ behavior: 'instant', block: 'center' });
                    return true;
                }
            }
        ...
        if (found) {
            await page.waitForTimeout(500);
            console.log('📍 Scrolled to Total Benefits row');
            return;
        }
    ...
    // Fallback: scroll to bottom if Total Benefits not found
    await page.evaluate(() => { window.scrollTo(0, document.body.scrollHeight); });
    await page.waitForTimeout(500);
    console.log('⚠️ Total Benefits row not found, scrolled to bottom');
}
...
for (const category of categories) {
    for (const level of levels) {
        for (const location of locationList) {
            ...
            test(`Scenario : ${num} | TotalFixed+CTC (WithScroll) | Category: ${category} | Level: ${level} | Location: ${location}`, async ({ page }) => {
                ...
                // ─── Scroll until Total Benefits row is visible ─────
                await scrollToTotalBenefits(page);
